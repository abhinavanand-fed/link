...
const DATA_PATH = process.env.DATA_PATH || path.join(__dirname, 'data', 'links.json');

const store = new LinkStore(DATA_PATH);
...
async function route(req, res) {
  ...
  if (req.method === 'POST' && pathName === '/shorten') {
    ...
    if (!isValidHttpUrl(originalUrl)) {
      return send(res, 400, homePage('Please provide a valid URL that starts with http:// or https://'));
    }

    if (customCode && !isValidCustomCode(customCode)) {
      return send(res, 400, homePage('Custom code must be 3-32 chars: letters, numbers, - or _.'));
    }

    if (customCode && store.codeExists(customCode)) {
      return send(res, 409, homePage('That custom alias is already taken.'));
    }

    const expiresAt = toIsoOrNull(expiresAtInput);
    if (expiresAtInput && !expiresAt) {
      return send(res, 400, homePage('Expiration should be a valid date/time. Example: 2026-12-31T23:59'));
    }
  ...
  if (req.method === 'POST' && pathName === '/api/shorten') {
    ...
    const record = store.create({ originalUrl, customCode: customCode || null, title, expiresAt });
    return send(
      res,
      201,
      JSON.stringify({
  ...
  if (req.method === 'GET' && pathName.length > 1) {
    ...
    if (isExpired(record.expiresAt)) {
      return send(res, 410, homePage('This short link has expired.'));
    }
    ...
    store.addVisit(code, { ipAddress, userAgent, referer });
